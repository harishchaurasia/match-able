...
const deiOptions = [
  "Wheelchair Accessibility (Accessible building entrances, restrooms, workspaces)",
  "Accessible Parking (Reserved parking spaces near the building)",
  // Add other DEI options here as necessary
];
...
const ProfilePage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    console.log("Selected DEI Needs:", selectedDeiNeeds);
    await uploadResume(); // Upload the resume when submitting the form
    router.push("/JobListingPage");
  };
...
//           Authorization: `Bearer ${token}`,
//         },
//         body: JSON.stringify({ name: demographics.name }),
//       });
...
//   const handleSubmit = async () => {
//     console.log("Selected DEI Needs:", selectedDeiNeeds);
//     await updateName(); // Update the user's name in the database
//     await uploadResume(); // Upload the resume when submitting the form
//     router.push("/JobListingPage");
//   };
