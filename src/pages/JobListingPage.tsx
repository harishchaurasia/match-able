...
const JobListingPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchJobsAndScores = async () => {
      try {
        // First, try to fetch companies data
        const companiesResponse = await axios.get(
          "http://localhost:3000/api/companies"
        );
        console.log("Companies response:", companiesResponse.data);

        // Then fetch similarity scores with hardcoded email
        const scoresResponse = await axios.get(
          "http://localhost:3000/calculate-similarity-test"
        );
        ...
        const sortedJobs = jobData.sort(
          (a: { matchPercentage: number }, b: { matchPercentage: number }) =>
